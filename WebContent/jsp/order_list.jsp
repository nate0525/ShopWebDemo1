			...
			<div style="margin: 0 auto; margin-top: 10px; width: 950px;">
				...
				<table class="table table-bordered">
					<c:forEach items="${page.list }" var="o">
						<tbody>
							<tr class="success">
								<th colspan="5">订单编号:${o.oid }&nbsp;总金额:￥${o.total } <c:if
										test="${o.state==1}">
										<a
											href="${pageContext.request.contextPath}/OrderServlet?method=findOrderByOid&oid=${o.oid}">付款</a>
									</c:if> <c:if test="${o.state==2}">未发货</c:if> <c:if
										test="${o.state==3}">已发货</c:if> <c:if test="${o.state==4}">已签收</c:if>
		...
		$(".delete")
				.click(
						function() {
							if (confirm("确认删除?")) {
								var oid = this.id;
								window.location.href = "/WebProject/OrderServlet?method=delOrderByOid&oid="
										+ oid;
							}
						});
	});
